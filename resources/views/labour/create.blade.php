...
@section('content')
    <div class="content-wrapper">
        @include('widgets.breadcrumb', ['title' => __('Labour Management'), 'subtitle'=> __('Labour'), 'button' => __('')])
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">{{__('Labour create')}}</h4>
                            </div>
                            <div class="card-body">
                                <form action="{{route('labour.store')}}" method="post" class="form">
                                    @csrf
                                    <div class="row">
                                                ...
                                                <label class="form-label" for="select2-basic">{{ __("Select Tender") }} <span class="text-danger">*</span></label>
                                                    <select class="select2 form-select" id="select2-basic" name="tender">
                                                        ...
                                                        @foreach ($tenders as $tender )
                                                            <option value="{{ $tender->id.'-'.$tender->tender_no }}">{{ $tender->name }}</option>
                                                        @endforeach
                                                        @endif
                                                    </select>
